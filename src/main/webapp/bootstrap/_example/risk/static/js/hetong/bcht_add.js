...
$(function(){
    ...
    var config = {
        container:"info",
        columns:[
            {col:"bcht",label:"补充合同ID",type:"text",type:"hidden"},
            {col:"htbh",label:"合同编号",type:"text",validate:{required:true}},
            {col:"htmc",label:"合同名称",type:"text",validate:{required:true}},
            {col:"signTime",label:"签约日期",type:"date",validate:{required:true,date:true}},
            {col:"startTime",label:"生效日期",type:"date",validate:{required:true,date:true,dateGEInput:"signTime"}},
            {col:"state",label:"有效标志",type:"select",validate:{required:false},
                options:[
                    {
                        value:"1",text:"有效"
                    },
    ...
    $("input[name='wtfmc']").next("a").click(function(){
        openLayer({
            ...
            area : ["98%","96%"],
            url : "../../hetong/buchonghetong/bcht_ghs_list.html"
        });
    });
