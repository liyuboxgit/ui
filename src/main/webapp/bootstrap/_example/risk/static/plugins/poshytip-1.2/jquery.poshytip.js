...
(function($) {
	...
	$.Poshytip = function(elm, options) {
		...
		this.$tip = $(['<div name="'+(this.$elm.attr("data_type")||this.$elm.attr("name")||'')+'" class="',this.opts.className,'">',
		...
		var $tab = this.$elm.closest("div.tab-pane");
		var id = $tab.attr("id");
		if($tab.length>0){
			var poshtipTargets = $tab.data("poshtipTargets");
			if(poshtipTargets==null){
				poshtipTargets = [];
				$tab.data("poshtipTargets",poshtipTargets);
			}
			poshtipTargets.push(this.$tip);
		}
	};
